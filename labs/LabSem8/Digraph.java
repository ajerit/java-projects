...
public class Digraph {
    ...
    // Retorno el costo del lado correspondiente
    public int costo(int v, int w) {
        return costos[v][w];
    } 
     ...
     */
    public void addEdge(int v, int w, int costo) {
        validateVertex(v);
        validateVertex(w);
        Arc e = new Arc(v, w, costo);
        lados.add(e);
        adj[v].add(w);
        indegree[w]++;
        costos[v][w] = costo;
    ...
    // Retorna una lista con todos los lados.
    public LinkedList<Arc> lados() {
        return lados;
    }
     ...
     * @throws IndexOutOfBoundsException si el vertice no pertenece al grafo
     */
	public LinkedList<Integer> getListPred(int v) {
		validateVertex(v);
     ...
     */
	public HashSet<HashSet<Integer>> componentesConexas(){
		// calcular matriz ady
